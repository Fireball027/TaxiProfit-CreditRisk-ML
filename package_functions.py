...
def variables(month, day, hour_min, hour_max, ntot, nrand):
    ...
    if month == 1 or month == 2:
        a = cursor.execute("""SELECT  a.[medallion], a.[hack_license], DATETIME(dw, a.[pickup_datetime]) as date,
        DATE PART(hh,a.[pickup_datetime]) as hour, a.[passenger_count], a.[trip_time_in_secs], a.[trip_distance], 
        a.[pickup_longitude], a.[pickup_latitude], b.[ total_amount], b.[ tip_amount] FROM [master].[dbo].[
        trip_data_""" + str(month) + """] AS a
...
ON  a.[medallion] = b.[medallion] AND a.[ hack_license]=b.[ hack_license] AND a.[ pickup_datetime]=b.[ pickup_datetime] 
    ...
    while len(X) < ntot:
        row = a.fetchone()
        if row:
            row = [row[0], row[1], row[2], float(row[3]), float(row[4]), float(row[5]), float(row[6]), float(row[7]),
                   float(row[8]), float(row[9]), float(row[10])]
            if hour_max >= row[3] >= hour_min and row[2] == day and row[5] > 0 and x_max > row[7] > x_min and y_max > row[8] > y_min:
...
def plotation(clf_list):
    ...
    for _, clf in enumerate(clf_list):
        ...
        for _, clfdata in enumerate([clfY, clfZ]):
            ...
            m = Basemap(llcrnrlon=x_min, llcrnrlat=y_min, urcrnrlon=x_max, urcrnrlat=y_max, resolution='i',
                        projection='cass', lon_0=-74.00597, lat_0=40.71427, ax=axes)
            m.drawcoastlines()
