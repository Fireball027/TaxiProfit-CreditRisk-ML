...
for i in range(len(X_tot)):
    long = X_tot[i][4]
    lati = X_tot[i][5]
    if -73.936854 <= long <= -73.814991 and 40.813647 <= lati <= 40.894584:
        X_tot[i][6] = 2
        print('Bronx')
    elif -73.936854 <= long <= -73.751520 and 40.704819 <= lati <= 40.775766:
        X_tot[i][6] = 3
        print('Queens')
    elif -74.041629 <= long <= -73.858295 and 40.572734 <= lati <= 40.704819:
        X_tot[i][6] = 4
        print('Brooklyn')
    elif -74.222290 <= long <= -74.054728 and 40.510196 <= lati <= 40.68754:
        X_tot[i][6] = 5
        print('Staten Island')
    elif -74.017371 <= long <= -73.936854 and 40.704819 <= lati <= 40.813647:
        X_tot[i][6] = 1
        # print('Manhattan')
    else:
        X_tot[i][6] = 0
...
# Decision Tree
# 1. Regression : Amount explained by trip time, trip distance, start time, passengers count
...
# 2. Regression: Distance explained by longitude, latitude
...
for n_estimators in range(1, 30):
    clf3Y = RandomForestRegressor(n_estimators=n_estimators, max_depth=max_depthY, min_samples_leaf=10).fit(
        X_train_reg3, Y_train)
    Y_test_pred, Y_train_pred = clf3Y.predict(X_test_reg3), clf3Y.predict(X_train_reg3)
    errtrY += [mean_squared_error(Y_train_pred, Y_train) ** 0.5]
    errteY += [mean_squared_error(Y_test_pred, Y_test) ** 0.5]
    clf3Z = RandomForestRegressor(n_estimators=n_estimators, max_depth=max_depthZ, min_samples_leaf=10).fit(
        X_train_reg3, Z_train)
...
n_estimatorsY = 7
n_estimatorsZ = 5
clf3Y = RandomForestRegressor(n_estimators=n_estimatorsY, max_depth=max_depthY, min_samples_leaf=10).fit(X_train_reg3,
                                                                                                         Y_train)
clf3Z = RandomForestRegressor(n_estimators=n_estimatorsZ, max_depth=max_depthZ, min_samples_leaf=10).fit(X_train_reg3,
                                                                                                         Z_train)
...
for n_estimators in range(1, 20):
    clf4T = RandomForestRegressor(n_estimators=n_estimators, max_depth=max_depthT, min_samples_leaf=10).fit(
        X_train_reg4, T_train)
    clf4D = RandomForestRegressor(n_estimators=n_estimators, max_depth=max_depthD, min_samples_leaf=10).fit(
        X_train_reg4, D_train)
...
n_estimatorsY = 15
n_estimatorsZ = 15
clf4Y = RandomForestRegressor(n_estimators=n_estimatorsY, max_depth=max_depthY, min_samples_leaf=10).fit(X_train_reg4,
                                                                                                         Y_train)
clf4Z = RandomForestRegressor(n_estimators=n_estimatorsZ, max_depth=max_depthZ, min_samples_leaf=10).fit(X_train_reg4,
                                                                                                         Z_train)
...
X_train_reg5 = np.transpose(
    [np.transpose(X_train)[i] for i in (0, 1, 2, 3)])    # hour, passenger_count, triptime, tripdistance
...
for i in range(1, 5):
    clf5Y = Regressor(layers=[Layer("Sigmoid", units=10 * i), Layer('Linear')], learning_rate=0.02, n_iter=10).fit(
        X_train_scaled, Y_train)
    clf5Z = Regressor(layers=[Layer("Sigmoid", units=10 * i), Layer('Linear')], learning_rate=0.02, n_iter=10).fit(
...
clf5Y = Regressor(layers=[Layer("Sigmoid", units=unitsY), Layer('Linear')], learning_rate=0.02, n_iter=10).fit(
    X_train_scaled, Y_train)
clf5Z = Regressor(layers=[Layer("Sigmoid", units=unitsZ), Layer('Linear')], learning_rate=0.02, n_iter=10).fit(
    X_train_scaled, Z_train)
...
plotation(
    [DecisionTreeRegressor(max_depth=10), RandomForestRegressor(n_estimators=10, max_depth=10, min_samples_leaf=10),
